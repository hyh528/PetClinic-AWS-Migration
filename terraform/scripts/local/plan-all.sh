...
# =============================================================================
# Terraform Plan All Layers (의존성 고려)
# =============================================================================
# 목적: 모든 레이어를 의존성 순서대로 plan 실행
...
plan_all_layers() {
    ...
    log "전체 레이어 Plan 시작 - 환경: $ENVIRONMENT"
    ...
    # Phase 1: 기본 인프라 (ALB 의존성 없음)
    log "=========================================="
    log "Phase 1: 기본 인프라 레이어 Plan"
    log "=========================================="
    
    for layer in "${PHASE1_LAYERS[@]}"; do
        if plan_layer "$layer" "Phase 1"; then
    ...
    if [[ ${#failed_layers[@]} -eq 0 ]]; then
        log "=========================================="
        log "Phase 2: ALB 의존성 레이어 Plan"
        log "=========================================="
        
        for layer in "${PHASE2_LAYERS[@]}"; do
            if plan_layer "$layer" "Phase 2"; then
                success_layers+=("$layer")
            else
                failed_layers+=("$layer")
            fi
    ...
    if [[ ${#failed_layers[@]} -gt 0 ]]; then
        ...
        log_success "모든 레이어 Plan 성공!"
        log "다음 단계: ./scripts/apply-all.sh $ENVIRONMENT"
        return 0
    fi
}
...
모든 Terraform 레이어를 의존성 순서대로 plan 실행합니다.
...
main() {
    ...
    log "Terraform Plan All 스크립트 시작"
    log "환경: $ENVIRONMENT"
    ...
    # Plan 실행
    if plan_all_layers; then
        log_success "전체 Plan 완료"
        exit 0
    else
        log_error "Plan 실패"
        exit 1
