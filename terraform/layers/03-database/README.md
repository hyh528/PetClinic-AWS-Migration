...
serverlessv2_scaling_configuration {
  # 개발 환경: min_capacity = 0.5, max_capacity = 1.0
  # 프로덕션 환경: min_capacity = 1.0, max_capacity = 4.0
}
```

**ACU (Aurora Capacity Unit)**: 약 2GB RAM + CPU
- **개발 환경**: 0.5-1.0 ACU (비용 절감)
- **프로덕션 환경**: 1.0-4.0 ACU (성능 우선)
...
2. RDS Data API 사용 (VPC 없이 HTTP로 접근)
    ...
    {
      "resourceArn": "arn:aws:rds:...:cluster:petclinic-dev",
...
**활성화 방법**: 클러스터 생성 후 자동으로 AWS CLI 호출
```bash
...
# 모니터링
performance_insights_enabled          = true
performance_insights_retention_period = 7  # 7일간 보존
...
- Enhanced Monitoring IAM 역할 1개 (모니터링 활성화 시)
- RDS Data API 자동 활성화 (null_resource)
...
| 구성 요소 | 개발 환경 | 프로덕션 환경 | 월 비용 (USD) |
|----------|-----------|---------------|---------------|
| Writer 인스턴스 | 0.5-1.0 ACU | 1.0-4.0 ACU | $20-80 / $40-320 |
| Reader 인스턴스 | 0.5-1.0 ACU | 1.0-4.0 ACU | $20-80 / $40-320 |
| Storage | 50GB | 100GB | $5.75 / $11.50 |
| I/O | 1M requests | 10M requests | $0.20 / $2.00 |
| 백업 (자동) | 50GB | 100GB | $0 (무료) |
| 백업 (수동) | 100GB | 500GB | $2.30 / $11.50 |
| **합계 (최소)** | - | - | **$48** / **$95** |
| **합계 (평균)** | - | - | **$75** / **$200** |
...
- Writer 인스턴스 1개 (db.serverless)
- Reader 인스턴스 1개 (db.serverless)
- Secrets Manager 시크릿 1개 (비밀번호 자동 관리)
- DB 서브넷 그룹 1개
- Enhanced Monitoring IAM 역할 1개 (선택적)
- RDS Data API 활성화 (null_resource)
