...
output "routing_configuration" {
  ...
  value = {
    base_url = aws_api_gateway_stage.this.invoke_url
    routes = {
      for service_name, service_config in local.all_services : service_name =>
      service_config.parent_path == "api" ?
      "${aws_api_gateway_stage.this.invoke_url}/api/${service_config.path}" :
      "${aws_api_gateway_stage.this.invoke_url}/${service_config.path}"
    }
    service_details = {
      for service_name, service_config in local.all_services : service_name => {
        path             = service_config.path
        ...
        full_url         = service_config.parent_path == "api" ? "${aws_api_gateway_stage.this.invoke_url}/api/${service_config.path}" : "${aws_api_gateway_stage.this.invoke_url}/${service_config.path}"
      }
    }
...
output "health_check_urls" {
  ...
  value = {
    for service_name, service_config in local.all_services : service_name =>
    service_config.parent_path == "api" ?
    "${aws_api_gateway_stage.this.invoke_url}/api/${service_config.path}/actuator/health" :
    "${aws_api_gateway_stage.this.invoke_url}/${service_config.path}/actuator/health"
  }
}
