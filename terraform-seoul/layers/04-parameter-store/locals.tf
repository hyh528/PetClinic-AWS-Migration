...
locals {
  ...
  database_parameters = local.dependencies_ready ? {
    ...
    "/petclinic-seoul/${var.environment}/db/url"      = "jdbc:mysql://${local.aurora_endpoint}:3306/petclinic?useSSL=false&allowPublicKeyRetrieval=true"
    "/petclinic-seoul/${var.environment}/db/username" = var.database_username
    # Secret ARN은 일반 String 파라미터로 저장 (SecureString이 아님)
    "/petclinic-seoul/${var.environment}/db/secrets-manager-name" = data.terraform_remote_state.database.outputs.master_user_secret_name
  } : {}
